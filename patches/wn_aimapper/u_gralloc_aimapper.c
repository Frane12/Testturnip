...
md_take_str(struct md_reader *r, char *out, size_t out_size)
{
   ...
   size_t copy = out_size ? MIN2((size_t)len, out_size - 1) : 0;
   if (out_size) {
      memcpy(out, r->p, copy);
      out[copy] = '\0';
   }
                               ...
                               struct u_gralloc_buffer_basic_info *out)
{
   ...
   for (int64_t i = 0; i < num_planes && r.ok; i++) {
      ...
      if (offset_bytes < 0 || offset_bytes > INT_MAX ||
          stride_bytes <= 0 || stride_bytes > INT_MAX)
         goto out_free;
   ...
   /* This port only normalizes known packed RGB layouts. Never reinterpret
    * arbitrary vendor compression or multi-plane YUV as Qualcomm UBWC.
    */
   const bool packed_rgb =
      fourcc == DRM_FORMAT_ABGR8888 || fourcc == DRM_FORMAT_XBGR8888 ||
      fourcc == DRM_FORMAT_ARGB8888 || fourcc == DRM_FORMAT_XRGB8888 ||
      fourcc == DRM_FORMAT_RGB565 || fourcc == DRM_FORMAT_ABGR2101010 ||
      fourcc == DRM_FORMAT_ABGR16161616F;
   const bool explicit_ubwc = modifier == DRM_FORMAT_MOD_QCOM_COMPRESSED;
   if (packed_rgb && num_planes == 2 && layout_says_ubwc &&
       (modifier == DRM_FORMAT_MOD_LINEAR ||
        modifier == DRM_FORMAT_MOD_INVALID || explicit_ubwc)) {
   ...
   /* Mesa 26.2.3 does not carry layer_count/alloc_size in this interface. */
   uint64_t layer_count = 0;
   if (md_scalar(gr, hnd->handle, SMT_LAYER_COUNT, &layer_count,
                 sizeof(layer_count)) && layer_count > 1)
      goto out_free;
...
{
   const char *option = os_get_option("TU_FRANE_AIMAPPER");
   if (option && strcmp(option, "0") == 0)
      return NULL;
